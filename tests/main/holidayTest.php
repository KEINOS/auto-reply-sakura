 ...
 * ----------------------------------------------------------------------------
 * About this test:
 *   The main script "auto-reply.php" receives a raw email data from the STDIN.
 *   Then replies an email if the send time is in range of time specified in
 *   the configuration file.
 *
 *   To test it's function, this test does the following:
 *     1. The "setUp()" method emulates the client action. Such as sending an
 *        email to the dummy SMTP server. Then it fetches back from the dummy
 *        SMTP server's API.
 *     2. In the test method(s), pipes the fetched raw email to the main script.
 *     3. The main script should send back (reply) to the sender's email address
 *        if received time is in between the range.
 ...
 *     And should be up in the same network and accessible to it's API as
...
final class Main_HolidayTest extends TestCase
{
    // ========================================================================
    //  Properties
    // ========================================================================
    ...
    public function testReceiveMailAndPipeToTheMainScript()
    {
        ...
        $time_stamp = \strtotime('2020/04/13 18:00'); // Monday. Holiday begins from 18:00

        // Add slash to escape to include in command
        $user_mail_raw = \addslashes($user_mail_raw);

        // Create command
        //   Debug mode is set to "true" with the above time stamp.
        //   In debug mode, the conf file is "./tests/data_dummy/config.sample1_regular.json"
        ...
        $result_status = false;
        $result_last_line = $this->exec($command, $result_status);

        // In debug mode, the script will return:
        //   If replied: the trace ID which was attached to the subject in the sent email.
        //   If not replied: A message that describes it didn't send any reply email.
        $id_trace = \trim($result_last_line);
        ...
        // Prepare error msg
        $msg_error = "Reply body does not match to the one sent.\n---\n${body_actual}\n---\n${body_expect}\n---";
        $this->assertSame($body_actual, $body_expect, $msg_error);
